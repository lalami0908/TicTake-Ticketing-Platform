...
function MyTicketsPageContent() {
  ...
  React.useEffect(() => {
    ...
    tickets.forEach((ticket) => {
      var activity = activities.filter(function (item) {
        return item.activity_id == ticket.activity_id;
      });
    ...
    actuall_tickets.sort((b: TicketType, a: TicketType) =>
      a.order_timestamp > b.order_timestamp
        ? 1
        : b.order_timestamp > a.order_timestamp
        ? -1
        : 0,
    );
  ...
  const clickToPayOrder = async (ticket: any) => {
    if (ticket.has_paid == 0) {
      ...
      var found = tickets.filter(function (item) {
        return item.key == ticket.key;
      });
      var after_tickets = tickets.filter(function (item) {
        return item.key != ticket.key;
      });
      if (found.length != 0) {
        found[0]['has_paid'] = '1';
          ...
          },
          {
            ...
            Cell: ({ cell }: CellProps<Event>) => (
              <div
                style={{
                  textDecoration: `${
                    cell.row.values.has_paid == 0 ? 'underline' : ''
                  }`,
                }}
                onClick={() => {
                  clickToPayOrder(cell.row.values);
                }}
              >
